...
for (test_id, test) in  enumerate(tests):
    ...
    for i in range(0, N):
        ...
        if(test['controller']=='IC_O_simpl'):
            tau[:,i] = h + J.T.dot(K.dot(x_ref[:,i] - x[:,i]) + B * (dx_ref[:,i] - dx[:,i]))    

        elif(test['controller']=='IC_O_simpl_post'):
            tau[:,i] = h + J.T.dot(K.dot(x_ref[:,i] - x[:,i]) + B * (dx_ref[:,i] - dx[:,i])) + NJ.dot(tau_0)

        elif(test['controller']=='IC_O'):
            tau[:,i] = J.T.dot(K.dot(x_ref[:,i] - x[:,i]) + B * (dx_ref[:,i] - dx[:,i]) + mu)

        elif(test['controller']=='IC_O_post'):                                          
            tau[:,i] = J.T.dot(K.dot(x_ref[:,i] - x[:,i]) + B * (dx_ref[:,i] - dx[:,i]) + mu) + NJ.dot(tau_0 + h)
